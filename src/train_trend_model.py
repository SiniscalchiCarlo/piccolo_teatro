...
def trend_error(df, model, start_offset, train_config, plot=True):
    ...
    for performance_id in performances:
        ...
        target_df = Y_test.loc[start_day + pd.Timedelta(days=1):end_date]
        predictions = []
        ...
        for index, y in target_df.items():
            ...
            if train_config.tickets_cum_sum.enabled and target_type == "tickets":
                    updated_input_df = add_cumulative_sum(updated_input_df, ["tickets"])

            # Add moving avarages and shifted values
            if train_config.tickets_cum_sum_avg.enabled:
            ...
            if train_config.tickets.enabled and target_type == "tickets_cum_sum":
                new_value = float(prediction-input_row["tickets_cum_sum"].iloc[0])
                values = input_df["tickets"].tolist()
                values.append(new_value)
                updated_input_df["tickets"] = values

            if train_config.tickets_cum_sum.enabled and target_type == "tickets":
                new_value = float(prediction+input_row["tickets_cum_sum"].iloc[0])
                values = input_df["tickets_cum_sum"].tolist()
                values.append(new_value)
                updated_input_df["tickets_cum_sum"] = values

            
            if train_config.remaining_tickets.enabled:
                new_value = float(input_row["remaining_tickets"].iloc[0]+input_row["tickets_cum_sum"].iloc[0])
                values = input_df["remaining_tickets"].tolist()
                values.append(new_value)
                updated_input_df["remaining_tickets"] = values
